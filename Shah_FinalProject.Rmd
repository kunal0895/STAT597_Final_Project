...
write.csv(df_by_Continents, "Continent Level Data", row.names = FALSE)
```
...
write.csv(df_public_debt_country, "Country Level Data", row.names = FALSE)
```

#### Which country had the highest ever debt to GDP ratio from 1996 to 2016?

```{r}
arrange(df_public_debt_country, desc(Debt_as_pct_of_GDP))[1:5, ]

```

#### Japan had the highest, infact the highest for the past 5 years.

#### Which country had the lowest ever debt to GDP ratio from 1996 to 2016?

```{r}
arrange(df_public_debt_country, Debt_as_pct_of_GDP)[1:5, ]
```
